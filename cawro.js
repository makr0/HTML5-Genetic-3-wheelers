...
function cw_nextGeneration() {
  ...
  var newborn;
  cw_getChampions();
  cw_topScores.push({i:vm.gen_counter,v:cw_carScores[0].v,x:cw_carScores[0].x,y:cw_carScores[0].y,y2:cw_carScores[0].y2});
  plot_graphs();
...
function cw_kill() {
  ...
  current_car_index++;
  cw_killCar();
  if(current_car_index >= vm.generationSize) {
    cw_nextGeneration();
    current_car_index = 0;
  }
